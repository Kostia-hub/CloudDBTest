...
public class LeftPanelController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        filesTable.setOnMouseClicked(new EventHandler<MouseEvent>() { //кликаем мышькой по строкам
            @Override
    ...
    public void uploadToServer(ActionEvent actionEvent) throws IOException, NoSuchAlgorithmException, ClassNotFoundException { //Загрузить на сервер
        Path path = Paths.get(pathField.getText()).resolve(filesTable.getSelectionModel().getSelectedItem().getFilename());
        Network.sendMsg(new FileMessage(path));
        System.out.println("Файл отправлен на сервак");
    }
}
